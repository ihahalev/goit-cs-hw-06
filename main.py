...
def save_data(data:bytes) -> dict:
    ...
    Returns:
        dict: decoded parsed and updated data
    """
    try:
...
def send_data_to_socket(data:bytes):
    ...
    try:
        ...
        logging.info(f'Saved data: {response.decode()}')
...
def handle(sock: socket.socket, address: str):
    ...
    """
    try:
        logging.info(f'Connection established {address}')
        while True:
            received = sock.recv(1024)
            if not received:
                break
            logging.info(f'Data received: {received}')
            saved = save_data(received)
            sock.send(str(saved).encode())
    except Exception as e:
        logging.error(f"Unexpected error on socket server receive: {e}")
    finally:
        logging.info(f'Socket connection closed {address}')
        sock.close()
...
def run_socket_server(ip:str, port:int):
    ...
    sock.listen(10)
    logging.info(f'Start socket server {sock.getsockname()}')
    with cf.ThreadPoolExecutor(10) as client_pool:
        try:
            ...
            logging.error(f"Unexpected error on socket server run: {e}")
        finally:
            sock.close()
